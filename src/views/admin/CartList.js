...
export default function CartList(props) {
	const { total, cart, handleDelete, decrementCount, incrementCount, updateCartTotal } = props
									...
									<Row>
										<Col>
											<p className={language === 'zh-CN' ? 'modal_title_CN' : 'modal_title'} style={{ marginBottom: '0rem' }}>
												...
												<span className={language === 'zh-CN' ? 'font_CN_normal' : 'product_details_count'}>{countItemCart(cart)}</span>
											</p>
										</Col>
									</Row>
									{cart.map((cartdetail) => {
