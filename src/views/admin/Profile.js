...
import { Row, Col, Button, FormGroup, Form, InputGroup, Container, NavLink, Modal, Dropdown } from 'react-bootstrap'
...
export default function Profile(props) {
	...
	const validationSchema = Yup.object({
		...
		lastName: Yup.string()
			...
			.matches(regexChar, dictionary.not_special_char_number)
			.required(dictionary.required),
		password: Yup.string().min(6, dictionary.too_short).max(24, dictionary.too_long).matches(regexPassword, dictionary.password_regex),
		confirm_password: Yup.string()
			.min(6, dictionary.too_short)
			.max(24, dictionary.too_long)
			.oneOf([Yup.ref('password'), null], dictionary.password_match),
		address: Yup.string().min(2, dictionary.too_short).max(100, dictionary.too_long).matches(regexCharInteger, dictionary.no_special_char),
		city: Yup.string().min(2, dictionary.too_short).max(40, dictionary.too_long).matches(regexChar, dictionary.not_special_char_number),
		state: Yup.string().min(2, dictionary.too_short).max(40, dictionary.too_long),
		postcode: Yup.number().integer(dictionary.not_valid).positive(dictionary.not_valid).min(1, dictionary.too_short).max(1000000, dictionary.too_long),
									...
									<Row className='header_cart_right ml-md-2 justify-content-center'>
										<Col md={6} style={{ paddingTop: '2.4rem' }}>
											<Dropdown>
												...
												<Dropdown.Menu>
													<Dropdown.Item
														className={language === 'zh-CN' ? 'dropdown_item font_CN_normal' : 'dropdown_item'}
														as='button'
														onClick={handleChangeInfo}>
														<FontAwesomeIcon className='mr-2' size='1x' icon={['fas', 'edit']} />
													...
													<Dropdown.Item
														...
														<FontAwesomeIcon className='mr-2' size='1x' icon={['fas', 'lock']} />
													...
													<Dropdown.Item
														...
														as='button'
														onClick={handleShow}>
														<FontAwesomeIcon className='mr-2' size='1x' icon={['fas', 'trash-alt']} />
									...
									{changePassword && (
										<Container>
											<Row className='justify-content-center mt-5'>
												<Col md={11}>
													<p className={language === 'zh-CN' ? 'modal_subtitle_CN' : 'modal_subtitle'}>
															...
															<Form role='form' onSubmit={handleSubmit}>
																...
																<div className='text-center'>
																	<Button
																		...
																		{isDisabled && (
																			<ClipLoader css='margin-bottom: -3%; margin-left: 5%' color={'white'} size={15} />
																		)}
																	</Button>
																</div>
															</Form>
																	...
																	<Col xs={12}>
																		<FormGroup className='mb-3'>
																			...
																			{errors.address && touched.address && (
																				<div className='error_field'>{errors.address}</div>
																			)}
																		</FormGroup>
																	</Col>
																	<Col xs={12}>
																		<FormGroup className='mb-3'>
																			...
																			{errors.postcode && touched.postcode && (
																				<div className='error_field'>{errors.postcode}</div>
																			)}
																		</FormGroup>
																	</Col>
																	<Col xs={12}>
																	...
																	<Col xs={12}>
																		<FormGroup className='mb-3'>
																			...
																			{errors.country && touched.country && (
																				<div className='error_field'>{errors.country}</div>
																			)}
																		</FormGroup>
																	</Col>
																	<Col xs={12}>
			...
			<Modal size='sm' show={show} onHide={handleClose} animation={false} centered>
				<Container style={{ borderBottom: '1px solid #eae8e4' }}>
					<Row className='justify-content-center'>
						...
						<Col xs={2} onClick={handleClose} style={{ paddingTop: '1.2rem' }} className='text-center'>
					...
					<Row>
						<Col>
							<Button
								size='sm'
								variant='secondary'
								className={language === 'zh-CN' ? 'button_cancel font_CN_normal' : 'button_cancel font_CN_normal'}
								onClick={handleClose}>
								{dictionary.cancel}
							</Button>
						</Col>
						<Col>
							<Button
								className={language === 'zh-CN' ? 'button_cart font_CN_normal' : 'button_cart font_CN_normal'}
								style={{ height: '2.7rem' }}
								size='sm'
								color='danger'
								variant='primary'
