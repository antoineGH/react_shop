...
export default function Payment(props) {
			...
			.then(async function (responseJson) {
				...
				if (result.error) {
					...
					if (result.paymentIntent.status === 'succeeded') {
						...
						authFetch('https://flask-shop-application-api.herokuapp.com/api/order', {
							method: 'POST',
							headers: {
								'Content-Type': 'application/json',
							},
	...
	return (
		<>
			<Container fluid>
				<Row>
					<Col xs={{ span: 12, order: 2 }} xl={{ span: 8, order: 1 }} style={{ backgroundColor: '#f6f5f3' }}>
									...
									<Row>
										<Col>
											<p className={language === 'zh-CN' ? 'modal_title_CN' : 'modal_title'} style={{ marginBottom: '0rem' }}>
												...
												<span className={language ? 'font_CN_normal' : 'product_details_count'}>{countItemCart(cart)}</span>
											</p>
										</Col>
									</Row>
										...
										<Row style={{ padding: '1rem' }}>
											...
											<Col
												...
												<Card.Text
													className={language === 'zh-CN' ? 'font_normal_size_CN' : 'font_normal_size'}
													style={{ marginBottom: '0rem' }}>
													{delivery.state && titleCase(delivery.state)} {delivery.country && titleCase(delivery.country)}
												</Card.Text>
											</Col>
										</Row>
									</Card>
												...
												<Card className='mx-auto mb-3 card_cart_product'>
													<Row className='no-gutters'>
														<Col md={3} className='my-auto'>
															<Image className='card_product_image_payment' src={cartdetail.images_url} />
														</Col>
															...
															<Card.Body>
																...
																<Row className='text-center text-md-left'>
																	<Col style={{ marginTop: '0', marginBottom: '0' }}>
																		<Card.Text className={language === 'zh-CN' ? 'font_CN_normal' : ''}>
							...
							<Row>
								<Col md={11}>
									<Row className='ml-md-2 justify-content-center'>
										<Col
											md={6}
											className={language === 'zh-CN' ? 'font_normal_size_plus_CN' : 'font_normal_size_plus'}
									...
									<Row className='ml-md-2 justify-content-center'>
										<Col md={12} className={language === 'zh-CN' ? 'font_normal_size_CN' : 'font_normal_size'}>
											<CardSection />
										</Col>
									</Row>
									<Row className='header_cart_right ml-md-2 justify-content-center'>
										<Col md={6}>
											<Button
												...
												{dictionary.pay}
												{'  '}
												{isDisabled && <ClipLoader css='margin-bottom: -0.8%; margin-left: 1.5%' color={'white'} size={15} />}
											</Button>
										</Col>
									</Row>
