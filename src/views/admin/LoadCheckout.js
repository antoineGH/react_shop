...
export default function LoadCheckout(props) {
	...
	function updateUserInfo(values, user_details_id, country) {
		...
		updateUserDetail(user_details_id, address, city, state, postcode, country, phone)
			.then((response) => {
				setIsLoaded(false)
				setAddress(address)
				setCity(city)
				...
				if (userDetailsCopy.length >= 1) {
					...
					setUserDetails([{ address: address, city: city, state: state, postcode: postcode, country: country, phone: phone }])
				}
	...
	function deleteUserDetail(user_details_id) {
		...
		const userDetailsUpdated = userDetails.filter((userDetail) => userDetail.user_details_id !== parseInt(user_details_id))
									...
									<Col md={12}>
										...
										<Row>
											<Col style={{ marginTop: '0rem', marginBottom: '0rem' }}>
												<p
													className={
														language ? 'font_normal_size_CN text-center text-md-left' : 'font_normal_size text-center text-md-left'
													}>
