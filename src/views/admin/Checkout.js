...
import { Container, Form, FormGroup, Row, Col, InputGroup, Button, Card } from 'react-bootstrap'
...
export default function Checkout(props) {
	...
	const [selectedOption, setSelectedOption] = useState({ value: country, label: country })
	...
	const validationSchema = Yup.object({
		...
		city: Yup.string()
			...
			.required(dictionary.required),
		state: Yup.string().min(2, dictionary.too_short).max(40, dictionary.too_long).required(dictionary.required),
		postcode: Yup.number()
			...
			.required(dictionary.required),
		phone: Yup.string().matches(phoneRegExp, dictionary.not_valid).required(dictionary.required),
	})
	...
	const { handleSubmit, handleChange, handleBlur, values, touched, errors } = useFormik({
		...
		onSubmit(values) {
			if (!deliveryOption) {
				fetchUserDetails()
					.then((response) => {
						updateUserInfo(values, response[0].user_details_id, selectedOption.value)
						return
					})
					.catch((error) => {
						console.log(error)
				...
				hasAddDetail ? addUserInfo(values, selectedOption.value) : updateUserInfo(values, deliveryOption, selectedOption.value)
			}
		},
	})

	function checkUserDetail() {
		if (address === null || city === null || country === null || phone === null || state === null || postcode === null) {
			return false
		}
		return true
	}
	...
	function chooseDelivery(event) {
		...
		const selectedUserDetail = userDetails.filter((userDetail) => userDetail.user_details_id === parseInt(event.target.value))
	...
	if (hasUserDetail) {
		return (
			<>
				<Container fluid>
					<Row>
						<Col xs={{ span: 12, order: 2 }} lg={{ span: 8, order: 1 }} style={{ backgroundColor: '#f6f5f3' }}>
								...
								<Row>
									<Col md={12}>
										...
										<Card className='mx-auto mb-3 card_cart_product'>
											<Form role='form' onSubmit={(event) => editUserDetail(event, deliveryOption)}>
												<Row style={{ padding: '2rem 1rem' }}>
													<Col xs={12} sm={12} md={9} lg={8} xl={8}>
														<Form.Control as='select' className='custom_dropdown' onChange={(event) => chooseDelivery(event)}>
															{userDetails.map((userDetail, count) => {
																count += 1
																return (
																	<option value={userDetail.user_details_id} key={count}>
																		{`${count} - ${titleCase(userDetail.address)}, ${userDetail.postcode} ${titleCase(
																			userDetail.city
																		)}, ${titleCase(userDetail.state)}, ${titleCase(userDetail.country)}.`}
																	</option>
																)
															})}
														</Form.Control>
								...
								<Row>
									<Col md={11} className='mb-4 mb-lg-3'>
										<Row className='header_cart_right justify-content-center'>
											<Col md={8} lg={12} xl={6} style={{ paddingTop: '2.4rem' }}>
												<Button
													...
													{dictionary.proceed}
													{'  '}
													<FontAwesomeIcon className='mr-2' size='1x' icon={['fas', 'chevron-right']} />
												</Button>
											</Col>
										</Row>
										<Col md={8} lg={12} xl={6} className='mx-auto'>
											<Row>
												<Col md={12}>
													<p
														className={
															language === 'zh-CN'
																? 'font_normal_size_plus_CN text-center text-md-left'
																: 'font_normal_size_plus text-center text-md-left'
														}
											...
											<Row>
												<Col xs={12} sm={12} md={12} lg={12} className='my-auto' style={{ marginBottom: '0' }}>
													<Card.Text
														className={
															language === 'zh-CN'
																? 'font_normal_size_CN text-center text-lg-left'
																: 'font_normal_size text-center text-lg-left'
														}
														style={{ marginBottom: '0rem', fontSize: '1.3rem' }}>
														{first_name && titleCase(first_name)} {last_name && titleCase(last_name)}
													</Card.Text>
												</Col>
											</Row>
											<Row>
											...
											<Row>
												<Col xs={12} sm={12} md={12} lg={12} className='my-auto' style={{ marginBottom: '0' }}>
													<Card.Text
														...
														style={{ marginBottom: '0rem', fontSize: '1.2rem' }}>
														{state && titleCase(state)}, {country && titleCase(country)}.
													</Card.Text>
												</Col>
											</Row>
										</Col>
		...
		return (
			<>
				<Container fluid style={{ minHeight: '90vh', backgroundColor: '#f6f5f3' }}>
					<Container>
						<Row>
							<Col md={12}>
								<Row className='header_cart ml-md-2'>
									...
									<Col className='mt-2 mt-sm-0' xs={12} sm={5} md={5} lg={4} xl={3}>
										<Button className='button_cart' style={{ height: '3.3rem' }} onClick={handleSubmit} size='sm' disabled={isDisabled}>
											{isDisabled && <ClipLoader css='margin-bottom: -1.5%; margin-right: 5%' color={'white'} size={15} />}
											{hasAddDetail ? dictionary.add_address : dictionary.update_address}
											{'  '}
								...
								<Row>
									<Col>
										<p className={language === 'zh-CN' ? 'modal_title_CN' : 'modal_title'} style={{ marginBottom: '0rem' }}>
											{dictionary.my_adress.toUpperCase()} <span className='product_details_count'></span>
										</p>
									</Col>
								</Row>
								<Row>
									<Col style={{ paddingBottom: '1rem' }}>
										<Card className='mx-auto mb-3 card_cart_product'>
											<Form role='form' onSubmit={handleSubmit} style={{ padding: '1rem' }}>
												<Row>
													<Col md={6}>
														<Row>
															<p
																className={language === 'zh-CN' ? 'label_form_CN ml-3' : 'label_form ml-3'}
														...
														<FormGroup className='mb-3'>
															...
															{errors.address && touched.address && <div className='error_field'>{errors.address}</div>}
														</FormGroup>
													</Col>
													<Col md={6}>
														<Row>
														...
														<FormGroup className='mb-3'>
															...
															{errors.city && touched.city && <div className='error_field'>{errors.city}</div>}
														</FormGroup>
													</Col>
												</Row>
												<Row>
													<Col md={6}>
														...
														<FormGroup className='mb-3'>
															...
															{errors.state && touched.state && <div className='error_field'>{errors.state}</div>}
														</FormGroup>
													</Col>
													<Col md={6}>
														<Row>
														...
														<FormGroup className='mb-3'>
															...
															{errors.postcode && touched.postcode && <div className='error_field'>{errors.postcode}</div>}
														</FormGroup>
													</Col>
												</Row>
												<Row>
													<Col md={6}>
														...
														<FormGroup className='mb-3'>
															<Select
																...
																options={options}
															/>
															{errors.country && touched.country && <div className='error_field'>{errors.country}</div>}
														</FormGroup>
													</Col>
													<Col md={6}>
														<Row>
														...
														<FormGroup className='mb-3'>
															...
															{errors.phone && touched.phone && <div className='error_field'>{errors.phone}</div>}
														</FormGroup>
													</Col>
												</Row>
											</Form>
