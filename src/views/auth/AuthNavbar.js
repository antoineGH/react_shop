...
import { Row, Col, Navbar, Nav, Button, DropdownButton, Dropdown } from 'react-bootstrap'
				...
				<Col xs={12} md={11}>
					<Row>
						<Col md={7} lg={8} xl={9} style={{ paddingRight: '0px' }}>
							<Navbar className='admin_navbar ml-sm-2' bg='light' variant='light' expand='lg'>
								<Link to='/'>
						...
						<Col sm={12} md={4} lg={3} xl={3} className='my-auto ml-0 ml-md-5 ml-xl-0' style={{ paddingRight: '0px' }}>
							<Navbar className='admin_navbar' expand='lg' variant='light' bg='light' style={{ marginRight: '0', boxShadow: 'none' }}>
								...
								{props.language !== 'en-US' ? (
									<Button className='button_language ml-5' size='sm' onClick={() => props.handleUpdateLanguage('en-US')}>
										<Flags.CN title='China' className='flag_cn mb-1' />
										<span className='ml-2 font_CN_normal'>中文</span>
									</Button>
								) : (
									<Button className='button_language ml-5' size='sm' onClick={() => props.handleUpdateLanguage('zh-CN')}>
